...
while (true) {
  if (o1.name == "Pennywise") {
    ...
    if (strategy.strategy == "Attack") {
      ...
      if (num == 0) {
        if (p1.fuel > 25) {
          p1.decreseFuel();
          console.log(
            `${chalk.bold.italic.blue(`${p1.name} has ${p1.fuel} fuel`)}`
          );
          console.log(
            `${chalk.bold.italic.blue(`${o1.name} has ${o1.fuel} fuel`)}`
          );
        } else if (p1.fuel <= 25) {
      ...
      if (num == 1) {
        if (o1.fuel > 25) {
          ...
          console.log(
            `${chalk.bold.green(`Your opponent ran out of fuel You Won`)}`
          );
          process.exit();
        }
      }
    ...
    if (strategy.strategy == "Drink Health") {
      if (p1.drinkAvailable) {
        ...
        console.log(
          `${chalk.bold.red(`You have already used your energy drink`)}`
        );
      }
    }
  ...
  if (o1.name == "The Riddler") {
    ...
    if (strategy.strategy == "Run") {
      console.log(
        `${chalk.bold.red(`${p1.name} you're a coward! YOU LOOOOOOSE`)}`
      );
      process.exit();
    }
  }
